...
Public form submissions: name, email, subject, message, plus `ip` and `user_agent`. Listed in the Avo sidebar under **Messages**. Submissions are emailed with `ContactMailer` as HTML + text (Mailpit in development, SMTP in production).
...
Transactional emails inline the **dark** tokens from `design_tokens.css` (`.dark`) in `app/views/layouts/mailer.html.erb`. Email clients ignore CSS variables, so the templates do not use Tailwind or `var(--ds-*)`. HTML mail is table-based with inline styles for Gmail, Outlook, Apple Mail, and Yahoo. A matching text part is always included. System fonts only (no Inter webfont).
...
All mailers inherit `ApplicationMailer` and the shared `mailer` layout (HTML + text). `Devise::Mailer` uses the same parent (`config.parent_mailer`). `ContactMailer#new_message` is multipart: it sets `Reply-To` to the visitor, links to the Avo contact record (`/admin/resources/contacts/:id`), and uses the profile name in the chrome when `Profile.current` exists. Password reset mail (`Devise::Mailer`) uses the same chrome plus branded HTML/text templates under `app/views/devise/mailer/`.
...
app/
  ...
  mailers/                ApplicationMailer, ContactMailer
  helpers/                seo_helper
  views/layouts/          mailer.html.erb, mailer.text.erb
  views/contact_mailer/   new_message.html.erb, new_message.text.erb
  views/devise/mailer/    branded reset password HTML + text
...
- **Colors:** design-system tokens. Avo, the site, and HTML emails share the same identity (emails inline the dark hex values).
